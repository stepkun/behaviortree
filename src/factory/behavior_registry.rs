...
impl BehaviorRegistry {
    ...
    pub fn add_behavior<F>(
        &mut self,
        bhvr_description: BehaviorDescription,
        bhvr_creation_fn: F,
    ) -> Result<(), Error>
    where
        F: Fn() -> BehaviorPtr + Send + Sync + 'static,
    {
        if self.behaviors.contains_key(bhvr_description.name()) {
            return Err(Error::BehaviorAlreadyRegistered(
    ...
    pub(crate) const fn behaviors(
        &self,
    ) -> &BTreeMap<ConstString, (BehaviorDescription, Arc<BehaviorCreationFn>)> {
        &self.behaviors
    }
    ...
    pub(crate) fn fetch(
        &self,
        id: &str,
    ) -> Result<(BehaviorDescription, Arc<BehaviorCreationFn>), Error> {
