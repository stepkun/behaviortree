...
use crate::{
    ...
    behavior::{
        ...
        control::{
            ...
            Sequence, SequenceWithMemory, Switch, WhileDoElse,
        },
        decorator::{
...
impl BehaviorTreeFactory {
    ...
    pub fn core_behaviors(&mut self) -> Result<(), Error> {
        ...
        let bhvr_creation_fn = Box::new(move || -> Box<dyn BehaviorExecution> {
            Box::new(EntryUpdated::new(BehaviorState::Skipped))
        });
        self.registry_mut()
        ...
        let bhvr_creation_fn = Box::new(move || -> Box<dyn BehaviorExecution> {
            Box::new(EntryUpdated::new(BehaviorState::Running))
        });
        self.registry_mut()
            .add_behavior(bhvr_desc, bhvr_creation_fn)?;
    ...
    pub fn test_behaviors(&mut self) -> Result<(), Error> {
        ...
        let bhvr_creation_fn = Box::new(move || -> Box<dyn BehaviorExecution> {
            Box::new(ForceState::new(BehaviorState::Failure))
        });
        self.registry_mut()
            .add_behavior(bhvr_desc, bhvr_creation_fn)?;
        ...
        let bhvr_creation_fn = Box::new(move || -> Box<dyn BehaviorExecution> {
            Box::new(ForceState::new(BehaviorState::Success))
        });
        self.registry_mut()
            .add_behavior(bhvr_desc, bhvr_creation_fn)?;
    ...
    pub fn create_tree_with(
        ...
        let mut parser = XmlParser::default();
        let root =
            parser.create_tree_from_definition(name, &mut self.registry, Some(blackboard))?;
        Ok(BehaviorTree::new(root, &self.registry))
    }
        ...
        T: Behavior,
    {
        ...
        self.registry.add_behavior(bhvr_desc, bhvr_creation_fn)
    }
