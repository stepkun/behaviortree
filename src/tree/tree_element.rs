...
impl BehaviorTreeElement {
    ...
    fn new(
        ...
        data.description_mut().set_groot2_path(groot2_path);
        Self {
            kind,
            behavior,
            data,
    ...
    {
        self.data.add_pre_state_change_callback(name, callback);
    }
    ...
    fn check_pre_conditions(
        ...
        if self.pre_conditions.is_some() {
            ...
            if self.data.state() == BehaviorState::Idle
                || self.data.state() == BehaviorState::Skipped
            {
                if let Some(script) = self.pre_conditions.get("_failureif") {
                    let res = runtime.lock().run(script, self.data.blackboard_mut())?;
            ...
            match state {
                BehaviorState::Failure => {
                    if let Some(script) = self.post_conditions.get("_onFailure") {
                        let _: Result<tinyscript::execution::ScriptingValue, tinyscript::Error> =
                            runtime.lock().run(script, self.data.blackboard_mut());
                    }
                }
