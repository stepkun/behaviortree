...
fn mock_action(behavior: &mut BehaviorData) -> BehaviorResult {
	info!(
		"MockAction substituting behavior with full_path(): {}",
		behavior.full_path().as_ref()
	);
...
fn mock_say_something(behavior: &mut BehaviorData) -> BehaviorResult {
	...
	info!("MockSaySomething: {}", msg.as_str());
	Ok(BehaviorState::Success)
}
...
async fn example() -> BehaviorTreeResult {
	...
	info!("----- Behaviors fullPath() -------");
	for behavior in tree.iter() {
		info!("{}", behavior.full_path().as_ref());
	}
	info!("\n------ Output (original) ------");
...
async fn main() {
	info!("running t15_behavior_mocking...");
	match example().await {
		Ok(_) => {
			info!("...succeeded!");
